...
// Reais por Extenso
// =========================
// (C)2018 Derlidio Siqueira
// =========================

function jExtenso(valor: any) : string
{
    ...
    let inteiras:string = "", decimais:string = "", focoTrabalho:string = "";
    let digito:string = "", tmp:string = "", parte:string = "", bloco:string = "", frase:string = "", ultimaPalavra:string = "";
    let separador:number = 0, blocosAnalisados: number = 0, pontoDecimal:number = 0;
    let posicao:number = 0, currentStep:number = 0;
   ...
   // Verifica se o parâmetro recebido é numérico...
    ...
    if (pontoDecimal != -1) // indexOf retorna -1 se não encontrar a string.
    {
    ...
    if (inteiras.length % 3 != 0) { inteiras = ("   ").substr(inteiras.length % 3) + inteiras; }
    if (decimais.length % 3 != 0) { decimais = ("   ").substr(decimais.length % 3) + decimais; }
            ...
            do
            {
                ...
                if (parte != "")
                {
                    ...
                    switch(blocosAnalisados)
                    {
                        ...
                        case 3: if (plural) {bloco = "Milhões ";} else {bloco = "Milhão "} break;
                        case 4: if (plural) {bloco = "Bilhões ";} else {bloco = "Bilhão "} break;
                    }
    ...
    if (inteiras != "")
    {
        if (inteiras != "Um ")
        {
            ...
            if (ultimaPalavra == "ilhão " || ultimaPalavra == "lhões ") {inteiras = inteiras + "de ";}
