...
function bindEvents() {
    $("#clone").click(function () {
        var p = $(".knob_panel")[0];
        var x = $(p).clone(true).appendTo("body");
    ...
    $(".v_knob")[0]['angle'] = 0;
    $(".v_knob").bind('mousewheel DOMMouseScroll', function (event) {
        var parent = $(this).parent();
        var scale = parseFloat($(parent).find(".scale").text());
        var value = parseFloat($(this).find(".cell_value").text());
    ...
    $(".s_knob")[0]['angle'] = 0;
    $(".s_knob")[0]['scale'] = 1;
    $(".s_knob").bind('mousewheel DOMMouseScroll', function (event) {
        if (event.originalEvent['wheelDelta'] > 0 || event.originalEvent['detail'] < 0) {
...
function getCell(panel) {
    Excel.run(function (context) {
        var range = context.workbook.getSelectedRange();
        range.load('address');
        range.load('values');
        return context.sync().then(function () {
            var cell = range.address.toString();
...
function setCell(panel) {
    Excel.run(function (context) {
        var cell = $(panel).find(".cell_address").text();
